...
doc: # Builds the manpage.
	$(SCDOC) <doc/brane.1.scd >brane.1

install: # Installs the release binary.
	$(INSTALL) -d \
		$(DESTDIR)$(PREFIX)/$(BINDIR)/ \
		$(DESTDIR)$(PREFIX)/$(MANDIR)/man1/
	$(INSTALL) -pm 0755 brane $(DESTDIR)$(PREFIX)/$(BINDIR)/
	$(INSTALL) -pm 0644 brane.1 $(DESTDIR)$(PREFIX)/$(MANDIR)/man1/
